//
//  DefaultTaskSuggestionEngine.swift
//  Zunlo
//
...
final class DefaultTaskSuggestionEngine: TaskSuggestionEngine {
    
    let calendar: Calendar
    let taskFetcher: UserTaskFetcherService
    
    init(calendar: Calendar, taskFetcher: UserTaskFetcherService) {
    ...
    public func highPriorityCount(on date: Date) async -> Int {
        ...
        do {
            let all = try await taskFetcher.fetchTasks(filteredBy: TaskFilter(isCompleted: false))
    ...
    public func topUnscheduled(limit: Int) async -> [UserTask] {
        do {
            let all = try await taskFetcher.fetchTasks()
            return all
