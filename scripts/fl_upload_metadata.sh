...
LOG_FILE="$PROJECT_DIR/logs/fastlane/upload_metadata_${ENV_ARG}_${TIMESTAMP}.log"

echo "===========================================" | tee "$LOG_FILE"
echo "Fastlane Lane: upload_metadata" | tee -a "$LOG_FILE"
...
# Run fastlane lane
cd "$PROJECT_DIR" && fastlane upload_metadata 2>&1 | tee -a "$LOG_FILE"
