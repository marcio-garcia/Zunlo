...
func dayPt() -> String { [
    ...
    "amanhã","dia 10","neste fim de semana","hoje à noite","no domingo de manhã","próxima semana","próxima terça","essa quarta","daqui a um mês"
    ].randomElement()! }
func nextDayPhraseEn() -> String { [
    "next \(dayEn())","tomorrow","this \(dayEn())","next week","next week \(dayEn())","\(monthEn()) \(num())th","later","on the \(num())th",
...
let objsEn = [
    "coffee with Ana","demo","team lunch","review","standup","office hours","yoga class","see mom",
    ...
    "presentation","lunch meeting","interview","wedding planning meeting","vet appointment","oil change","client meeting","yoga class"]
let objsPt = [
    "café com Ana","reunião de produto","almoço com o time","revisão","daily","plantão de dúvidas","aula de yoga","visitar mãe",
    ...
    "apresentação", "reunião para almoço", "entrevista", "reunião de planejamento do casamento", "consulta ao veterinário", "troca de óleo", "reunião com cliente", "aula de ioga"]
...
// multi-intent
"schedule {obj} {when} and remind me to {task}",
"book {obj} {when} and add {task} to my list",
"create event {obj} {when} and task {task}"
    ]
    let pt = [
...
// ambiguous
"adicione {obj} a ele",
"coloque {obj} lá também",
"altere para {when}",
"mova para {time}",
"atualize com {time}",

// multi-intent
"agendar {obj} {when} e me lembrar de {task}",
"reservar {obj} {when} e adicionar {task} à minha lista",
"criar evento {obj} {when} e tarefa {task}"
    ]
...
// multi-intent
"move {obj} to {when} and add {task}",
"reschedule {obj} to {when} with {task}",
"shift {obj} to {when} and change location",
    ]
    let pt = [
...
// multi-intent
"mover {obj} para {when} e adicionar {task}",
"reprogramar {obj} para {when} com {task}",
"deslocar {obj} para {when} e alterar o local",
...
"add attendees to the project review \(dayEn())",
"update doctor appointment \(dayEn()) with new address",
"add notes to the conference call event \(dayEn())",
...
// multi-intent
"what's my schedule and how should i plan today",
"show me today's agenda and help organize my day",
"display my calendar and plan my priorities",
    ]
    let pt = [
...
// multi-intent
"qual é a minha agenda e como devo planejar hoje",
"mostrar a agenda de hoje e ajudar a organizar meu dia",
"mostrar meu calendário e planejar minhas prioridades",
    ]
